...
const statusTones: Record<WarehouseOrderStatus, string> = {
  ...
  "Do weryfikacji": "bg-[#fff5e8] text-[#9a4a00]",
  Przyjęte: "bg-[#eef5ff] text-[#275fa6]",
  "W produkcji": "bg-danger-paper text-accent-ink",
  Gotowe: "bg-[#edf7f0] text-ok",
...
export function WarehouseCustomersScreen() {
  const [selectedId,setSelectedId]=useState<CustomerRowId>(customerRows[0].id); const selected=customerRows.find((item)=>item.id===selectedId)??customerRows[0]; const [discount,setDiscount]=useState<number>(selected.discount); const [saved,setSaved]=useState(false);
  function selectCustomer(id:string){const found=customerRows.find((item)=>item.id===id);if(!found)return;setSelectedId(found.id);setDiscount(found.discount);setSaved(false);}
  return <main id="main-content" className="mx-auto w-full max-w-[1320px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><WarehouseHeader eyebrow="Warunki handlowe" title="Klienci i rabaty" subtitle="Warunki klienta są jednym źródłem ceny widocznej w portalu. Klient nigdy nie widzi ceny bazowej ani osobnej pozycji rabatu." /><div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]"><section className="overflow-hidden rounded-core bg-surface ring-1 ring-hair shadow-[var(--lift-sm)]"><div className="divide-y divide-hair">{customerRows.map((customer)=><button key={customer.id} type="button" onClick={()=>selectCustomer(customer.id)} className={`grid w-full gap-3 px-5 py-4 text-left sm:grid-cols-[1.4fr_.5fr_.6fr_.7fr] sm:items-center ${selected.id===customer.id?"bg-paper shadow-[inset_3px_0_0_var(--color-accent)]":"hover:bg-paper/70"}`}><span><strong className="text-sm text-ink">{customer.name}</strong><small className="mt-1 block font-mono text-[10px] text-mute">{customer.id}</small></span><strong className="font-mono text-sm text-accent-ink">{customer.group} · {customer.discount}%</strong><span className="text-xs text-mute">{customer.payment}</span><span className={`text-xs font-semibold ${customer.status==="Aktywny"?"text-ok":"text-accent-ink"}`}>{customer.status}</span></button>)}</div></section><aside className="rounded-shell bg-shell p-1.5 ring-1 ring-hair"><div className="rounded-core bg-surface p-5 shadow-[var(--inner)]"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-mute">Warunki klienta</p><h2 className="mt-2 font-display text-2xl font-semibold text-ink">{selected.name}</h2><label className="mt-6 block text-xs font-semibold text-ink">Rabat dla cen portalowych: <strong className="font-mono text-accent-ink">{discount}%</strong><input type="range" min="0" max="20" value={discount} onChange={(event)=>{setDiscount(Number(event.target.value));setSaved(false);}} className="mt-3 w-full accent-[var(--color-accent)]" /></label><dl className="mt-5 divide-y divide-hair border-y border-hair text-xs"><div className="flex justify-between py-3"><dt className="text-mute">Grupa</dt><dd className="font-mono font-semibold text-ink">{selected.group}</dd></div><div className="flex justify-between py-3"><dt className="text-mute">Płatność</dt><dd className="font-semibold text-ink">{selected.payment}</dd></div><div className="flex justify-between py-3"><dt className="text-mute">Limit</dt><dd className="font-mono font-semibold text-ink">{selected.limit}</dd></div><div className="flex justify-between py-3"><dt className="text-mute">Obrót</dt><dd className="font-mono font-semibold text-ink">{selected.turnover}</dd></div></dl><button type="button" onClick={()=>setSaved(true)} className="mt-5 w-full rounded-full bg-accent px-5 py-3 text-sm font-semibold text-white">Zapisz warunki</button>{saved&&<p className="mt-3 rounded-ctl bg-[#edf7f0] p-3 text-xs font-semibold text-ok" aria-live="polite">Warunki zapisane lokalnie. Po integracji źródłem prawdy będzie system sprzedażowy.</p>}</div></aside></div></main>;
}

export function WarehouseIntegrationsScreen() {
  const tabs=[integrationCopy.mapping,integrationCopy.queue,integrationCopy.safeguards] as const; const [tab,setTab]=useState<(typeof tabs)[number]>(tabs[0]); const [message,setMessage]=useState("");
  return <main id="main-content" className="mx-auto w-full max-w-[1320px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><WarehouseHeader eyebrow={integrationCopy.eyebrow} title={integrationCopy.title} subtitle={integrationCopy.subtitle} action={<span className="w-fit rounded-full bg-danger-paper px-4 py-2.5 text-xs font-semibold text-accent-ink ring-1 ring-accent/10">{integrationCopy.disconnected}</span>} /><section className="mt-8 grid gap-4 lg:grid-cols-[1fr_auto]"><div className="rounded-core bg-ink p-6 text-white"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/50">Kontrakt integracji</p><h2 className="mt-3 font-display text-3xl font-semibold">Wersja {salesSystemContract.version}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-white/60">Klucze: SKU dla towaru, zewnętrzne ID kontrahenta oraz numer zamówienia Mussi. Wymagane operacje obejmują produkty, stany, warunki handlowe, zamówienia, statusy i dokumenty.</p></div><div className="flex flex-col gap-3 sm:flex-row lg:flex-col"><button type="button" onClick={()=>setMessage(integrationCopy.testResult)} className="rounded-full bg-surface px-5 py-3 text-sm font-semibold text-ink ring-1 ring-hair">{integrationCopy.test}</button><button type="button" onClick={()=>setMessage(integrationCopy.syncDemo)} className="rounded-full bg-accent px-5 py-3 text-sm font-semibold text-white">{integrationCopy.syncNow}</button></div></section>{message&&<p className="mt-4 rounded-ctl bg-[#edf7f0] p-4 text-sm font-semibold text-ok" aria-live="polite">{message}</p>}<nav className="mt-7 flex gap-2 overflow-x-auto" aria-label="Obszary integracji">{tabs.map((item)=><button key={item} type="button" onClick={()=>setTab(item)} className={`rounded-full px-4 py-2 text-xs font-semibold ${tab===item?"bg-ink text-white":"bg-surface text-mute ring-1 ring-hair"}`}>{item}</button>)}</nav><section className="mt-5 rounded-core bg-surface ring-1 ring-hair shadow-[var(--lift-sm)]">{tab===integrationCopy.mapping&&<div className="divide-y divide-hair">{integrationDomains.map((domain)=><article key={domain.name} className="grid gap-3 px-5 py-4 sm:grid-cols-[1.2fr_.7fr_1fr_.7fr] sm:items-center"><div><h3 className="text-sm font-semibold text-ink">{domain.name}</h3><p className="mt-1 font-mono text-[9px] text-mute">{domain.records}</p></div><span className="text-xs font-semibold text-mute">{domain.direction}</span><span className="font-mono text-[10px] text-ink">{domain.key}</span><span className={`text-xs font-semibold ${domain.status.includes("Wymaga")?"text-accent-ink":"text-ok"}`}>{domain.status}</span></article>)}</div>}{tab===integrationCopy.queue&&<div><div className="border-b border-hair bg-paper px-5 py-3 font-mono text-[9px] uppercase tracking-[0.1em] text-mute">Ostatnie zdarzenia demonstracyjne</div><div className="divide-y divide-hair">{syncEvents.map((event)=><article key={`${event.time}-${event.entity}`} className="grid gap-3 px-5 py-4 sm:grid-cols-[.45fr_.55fr_1fr_.7fr_1.3fr] sm:items-center"><time className="font-mono text-[10px] text-mute">{event.time}</time><span className="text-xs font-semibold text-ink">{event.type}</span><span className="font-mono text-[10px] text-ink">{event.entity}</span><strong className={`text-xs ${event.result==="Zaktualizowano"?"text-ok":"text-accent-ink"}`}>{event.result}</strong><span className="text-xs text-mute">{event.detail}</span></article>)}</div></div>}{tab===integrationCopy.safeguards&&<ol className="divide-y divide-hair">{integrationSafeguards.map((item,index)=><li key={item} className="grid gap-4 px-5 py-5 sm:grid-cols-[48px_1fr] sm:items-start"><span className="font-mono text-xs font-semibold text-accent-ink">{String(index+1).padStart(2,"0")}</span><p className="text-sm leading-6 text-ink">{item}</p></li>)}</ol>}</section></main>;
}
