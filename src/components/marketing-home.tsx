...
import { producenci } from "@/data/dekory";
...
export function MarketingHome({ produkty }: { produkty: ProductPreview[] }) {
  return (
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-hair bg-paper/92 backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 max-w-[1440px] items-center gap-4 px-4 sm:min-h-20 sm:px-6 lg:px-8">
          ...
          <Link href="/kreator" className="pressable flex min-h-11 items-center gap-2 rounded-full bg-accent py-1 pl-4 pr-1 text-xs font-semibold text-white sm:pl-5">Wyceń rozkrój <Arrow /></Link>
        </div>
      </header>

      <main id="main-content">
        <section id="top" className="mx-auto grid max-w-[1440px] gap-12 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[0.86fr_1.14fr] lg:items-center lg:px-8 lg:py-32">
          <div>
        ...
        <section aria-label="Marki w ofercie" className="border-y border-hair bg-surface py-6">
          <div className="mx-auto flex max-w-[1440px] gap-8 overflow-hidden px-4 font-display text-sm font-semibold text-mute sm:px-6 lg:px-8">{producenci.slice(0, 12).map((name) => <span key={name} className="shrink-0">{name}</span>)}</div>
        </section>
        ...
        <section id="oferta" className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
          ...
          <div className="mt-10 grid gap-4 lg:grid-cols-12">
            <article className="rounded-shell bg-surface p-7 ring-1 ring-hair shadow-[var(--lift-sm)] lg:col-span-7 lg:row-span-2 lg:p-10">
              ...
              <div className="mt-12 grid grid-cols-2 gap-3 sm:grid-cols-4">{["18 mm", "28 mm", "HDF 3 mm", "OSB 10–25"].map((item) => <span key={item} className="rounded-ctl bg-paper px-3 py-3 text-center font-mono text-xs text-ink">{item}</span>)}</div>
            </article>
